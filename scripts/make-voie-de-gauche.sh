...
# Le sous-titre d'origine (pastille blanche, y=208..348 en coords source) est
# recouvert par un bandeau à la charte portant le texte de Guillaume.
# Carton de fin : question d'engagement.
#
...
MASK=""
[[ "${1:-}" == "--mask" ]] && MASK=1
OUT="exports/08-voie-de-gauche${MASK:+-flou}.mp4"
mkdir -p "$T" work/gauche exports

# --- Textes ---
write() { printf '%s' "$2" > "$T/$1"; }
write h0.txt "Il n'y a que sur l'autoroute"
write h1.txt "qu'on est de gauche"
...
dt() { # fichier fontsize couleur y
  printf "drawtext=expansion=none:fontfile='%s':textfile='%s':fontcolor=%s:fontsize=%s:x=(w-text_w)/2:y=%s" \
    "$FB" "$T/$1" "$3" "$2" "$4"
}
...
CHAIN+=",$(dt h0.txt 64 "$BLANC" 150)"
CHAIN+=",$(dt h1.txt 64 "$BLANC" 238)"
CHAIN+=",drawbox=x=460:y=358:w=160:h=5:color=${ACCENT}@1:t=fill"
CHAIN+="[vout]"
